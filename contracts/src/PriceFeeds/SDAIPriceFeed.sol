...
contract SDAIPriceFeed is MainnetPriceFeedBase {
    ...
    function _fetchPricePrimary() internal returns (uint256, bool) {
        ...
        (uint256 sdaiDaiPrice, bool sdaiDaiOracleDown) = _getOracleAnswer(sdaiDaiOracle);
        ...
        if (sdaiDaiOracleDown) return (_shutDownAndSwitchToLastGoodPrice(address(sdaiDaiOracle.aggregator)), true);
        
        uint256 sdaiUSDPrice = FixedPointMathLib.mulWadUp(daiUSDPrice, sdaiDaiPrice);
