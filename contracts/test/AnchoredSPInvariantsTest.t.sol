...
contract AnchoredSPInvariantsTest is DevTestSetup {
    ...
    function invariant_allFundsClaimable() internal view {
        ...
        info("stabilityPoolEvro:          ", stabilityPoolEvro.decimal());
        info("claimableEvro:              ", claimableEvro.decimal());
        ...
        assertApproxEqAbsDecimal(stabilityPoolEvro, claimableEvro, 0.001 ether, 18, "SP BOLD !~ claimable BOLD");
        assertApproxEqAbsDecimal(yieldGainsOwed, sumYieldGains, 0.001 ether, 18, "SP yieldGainsOwed !~= sum(yieldGain)");

        //assertGe(stabilityPoolEvro, claimableEvro, "Not enough deposits for all depositors");
    ...
    function testUnclaimableDeposit() external {
        ...
        // totalEvroDeposits = 0.000001023636824878 ether

        // pulling `deposited` from fixture
        vm.prank(gabe);
        handler.provideToSp(98_009.398338924622100814 ether, false);

        // totalEvroDeposits = 98_009.398339948258925692 ether
        ...
        // totalEvroDeposits = 96_009.20655912634111747 ether
        // P = 0.979591836959510777 ether

        vm.prank(carl);
        handler.provideToSp(0.000000000001265034 ether, false);

        // totalEvroDeposits = 96_009.206559126342382504 ether

        vm.prank(fran);
        handler.liquidateMe();

        // totalEvroDeposits = 87_983.111274272549632173 ether
        ...
        // totalEvroDeposits = 89_983.303055094467442396 ether
        ...
        // totalEvroDeposits = 14_223.145246237857365034 ether
    ...
    function testUnclaimableDeposit2() external {
        ...
        // totalEvroDeposits = 9.398161381122161756 ether
        ...
        // totalEvroDeposits = 98_018.795421655094789016 ether

        vm.prank(hope);
        handler.provideToSp(89_926.427447073294525543 ether, false);

        // totalEvroDeposits = 187_945.222868728389314559 ether
        ...
        // totalEvroDeposits = 200_334.324808634021533966 ether

        // pulling `deposited` from fixture
        vm.prank(dana);
        handler.provideToSp(9_589.959513437908897281 ether, false);

        // totalEvroDeposits = 209_924.284322071930431247 ether
        ...
        // totalEvroDeposits = 158_240.665444895402868803 ether
        // P = 0.753798761091013085 ether

        vm.prank(barb);
        handler.provideToSp(0.000000000000008548 ether, false);

        // totalEvroDeposits = 158_240.665444895402877351 ether

        vm.prank(carl);
        handler.provideToSp(4_591.158415534017479187 ether, false);

        // totalEvroDeposits = 162_831.823860429420356538 ether

        vm.prank(dana);
        handler.provideToSp(86_019.232581553804992428 ether, false);

        // totalEvroDeposits = 248_851.056441983225348966 ether

        vm.prank(gabe);
        handler.provideToSp(83_736.829497136058174833 ether, false);

        // totalEvroDeposits = 332_587.885939119283523799 ether
        ...
        // totalEvroDeposits = 239_834.627348620891715052 ether
        ...
        // totalEvroDeposits = 287_664.972549246853986528 ether
        ...
        // totalEvroDeposits = 189_655.575288972881359771 ether
        ...
        // totalEvroDeposits = 91_646.178028698908732511 ether
        // P = 0.173176219343645801 ether

        vm.prank(hope);
        handler.liquidateMe();

        // totalEvroDeposits = 43_815.832828072946508866 ether
        // P = 0.082795163309295299 ether

        vm.prank(adam);
        handler.provideToSp(156_013.932831544173758454 ether, false);

        // totalEvroDeposits = 199_829.76565961712026732 ether

        vm.prank(barb);
        handler.provideToSp(149_177.525713798558063626 ether, false);

        // totalEvroDeposits = 349_007.291373415678330946 ether
        ...
        // totalEvroDeposits = 250_997.894113141705709824 ether
        ...
        // totalEvroDeposits = 273_794.248643028561279959 ether

        // pulling `deposited` from fixture
        vm.prank(fran);
        handler.provideToSp(3_365.431868318464668052 ether, false);

        // totalEvroDeposits = 277_159.680511347025948011 ether

        // pulling `deposited` from fixture
        vm.prank(eric);
        handler.provideToSp(6_369.148148716152063935 ether, false);

        // totalEvroDeposits = 283_528.828660063178011946 ether
        ...
        // totalEvroDeposits = 381_539.796057705953613574 ether

        vm.prank(dana);
        handler.provideToSp(13_294.811494145641399612 ether, false);

        // totalEvroDeposits = 394_834.607551851595013186 ether

        vm.prank(adam);
        handler.liquidateMe();

        // totalEvroDeposits = 321_571.005554027814550269 ether
        // P = 0.048495586543891854 ether

        vm.prank(gabe);
        handler.liquidateMe();

        // totalEvroDeposits = 257_257.842481930065273254 ether
        // P = 0.038796625779998194 ether

        vm.prank(gabe);
        handler.provideToSp(2_881.242711585620903523 ether, false);

        // totalEvroDeposits = 260_139.085193515686176777 ether

        vm.prank(barb);
        handler.liquidateMe();

        // totalEvroDeposits = 231_822.380699608727681032 ether
    ...
    function testUnderflow() external {
        ...
        // totalEvroDeposits = 9.397260273972700749 ether

        vm.prank(carl);
        handler.provideToSp(0.000000000000019902 ether, false);

        // totalEvroDeposits = 9.397260273972720651 ether

        // pulling `deposited` from fixture
        vm.prank(eric);
        handler.provideToSp(12.028493150685049425 ether, false);

        // totalEvroDeposits = 21.425753424657770076 ether

        // pulling `deposited` from fixture
        vm.prank(fran);
        handler.provideToSp(24.05698630137009885 ether, false);

        // totalEvroDeposits = 45.482739726027868926 ether

        // pulling `deposited` from fixture
        vm.prank(eric);
        handler.provideToSp(48.11397260274029571 ether, false);

        // totalEvroDeposits = 93.596712328768164636 ether

        vm.prank(hope);
        handler.provideToSp(22_378.224492402901169486 ether, false);

        // totalEvroDeposits = 22_471.821204731669334122 ether
        ...
        // totalEvroDeposits = 88_591.797721606710590587 ether
        ...
        // totalEvroDeposits = 187_610.16678988720866405 ether
        ...
        // totalEvroDeposits = 90_656.396374136004053808 ether
        ...
        // totalEvroDeposits = 1_300.23139899947145306 ether
        ...
        // totalEvroDeposits = 99_309.62865927344415383 ether
        ...
        // totalEvroDeposits = 197_319.0259195474168546 ether
        ...
        // totalEvroDeposits = 295_328.816909648620282017 ether
        ...
        // totalEvroDeposits = 384_946.949402676729154274 ether
        ...
        // totalEvroDeposits = 407_737.928743002003943159 ether
        ...
        // totalEvroDeposits = 309_728.531482728031337839 ether
    ...
    function testNotEnoughYieldToClaim() external {
        ...
        // totalEvroDeposits = 6.79318371710954749 ether

        vm.prank(barb);
        handler.provideToSp(54_410.610723992018269811 ether, false);

        // totalEvroDeposits = 54_417.403907709127817301 ether

        vm.prank(hope);
        handler.provideToSp(5_146.80395069777790841 ether, false);

        // totalEvroDeposits = 59_564.207858406905725711 ether
    ...
    function testNotEnoughYieldToClaim2() external {
        ...
        // totalEvroDeposits = 9.397269633285661087 ether
        ...
        // totalEvroDeposits = 9.39726963328568226 ether
        ...
        // totalEvroDeposits = 98_018.794529907258386793 ether

        vm.prank(barb);
        handler.liquidateMe();

        // totalEvroDeposits = 9.299656010243587514 ether
        ...
        // totalEvroDeposits = 98_018.696916284216292047 ether

        vm.prank(carl);
        handler.liquidateMe();

        // totalEvroDeposits = 9.299656010243685524 ether
        ...
        // totalEvroDeposits = 68_790.879153648718969545 ether

        // pulling `deposited` from fixture
        vm.prank(gabe);
        handler.provideToSp(39_486.108825255913132743 ether, false);

        // totalEvroDeposits = 108_276.987978904632102288 ether

        // pulling `deposited` from fixture
        vm.prank(fran);
        handler.provideToSp(98_009.403810376026620703 ether, false);

        // totalEvroDeposits = 206_286.391789280658722991 ether

        vm.prank(fran);
        handler.provideToSp(67_852.887887440994776149 ether, false);

        // totalEvroDeposits = 274_139.27967672165349914 ether

        vm.prank(adam);
        handler.provideToSp(78_134.913037086847714575 ether, false);

        // totalEvroDeposits = 352_274.192713808501213715 ether

        vm.prank(hope);
        handler.liquidateMe();

        // totalEvroDeposits = 312_788.083888552588080972 ether
        ...
        // totalEvroDeposits = 313_189.640716507470172702 ether
        ...
        // totalEvroDeposits = 244_408.061218868994888681 ether
        ...
        // totalEvroDeposits = 371_339.584252979675162290 ether

        vm.prank(fran);
        handler.liquidateMe();

        // totalEvroDeposits = 312_724.220142735330535195 ether
        ...
        // totalEvroDeposits = 220_675.594968675749714429 ether
        ...
        // totalEvroDeposits = 185_881.886526430367926095 ether
    ...
    function testNotEnoughYieldToClaim3() external {
        ...
        // totalEvroDeposits = 0.127035053107027317 ether
        ...
        // totalEvroDeposits = 52_006.537656254207288186 ether
        ...
        // totalEvroDeposits = 150_025.333077909302071776 ether

        vm.prank(gabe);
        handler.liquidateMe();

        // totalEvroDeposits = 52_015.935817635329467479 ether
        // P = 0.346714349839990399 ether

        vm.prank(hope);
        handler.liquidateMe();

        // totalEvroDeposits = 9.52519643422920661 ether
        ...
        // totalEvroDeposits = 45_512.659837343810727854 ether

        vm.prank(dana);
        handler.provideToSp(13_158.641347715694197298 ether, false);

        // totalEvroDeposits = 58_671.301185059504925152 ether

        vm.prank(fran);
        handler.provideToSp(88_720.671803804390427542 ether, false);

        // totalEvroDeposits = 147_391.972988863895352694 ether

        // pulling `deposited` from fixture
        vm.prank(barb);
        handler.provideToSp(98_224.246264626880042381 ether, false);

        // totalEvroDeposits = 245_616.219253490775395075 ether

        // pulling `deposited` from fixture
        vm.prank(fran);
        handler.provideToSp(2_000.191780821917808219 ether, false);

        // totalEvroDeposits = 247_616.411034312693203294 ether

        vm.prank(dana);
        handler.liquidateMe();

        // totalEvroDeposits = 149_392.164769685813259138 ether
        ...
        // totalEvroDeposits = 88_592.429118117287659518 ether
        ...
        // totalEvroDeposits = 105_323.133223692344418776 ether

        // pulling `deposited` from fixture
        vm.prank(dana);
        handler.provideToSp(2_755.636251036681598567 ether, false);

        // totalEvroDeposits = 108_078.769474729026017343 ether

        // pulling `deposited` from fixture
        vm.prank(carl);
        handler.provideToSp(528.278543964116931444 ether, false);

        // totalEvroDeposits = 108_607.048018693142948787 ether

        // pulling `deposited` from fixture
        vm.prank(eric);
        handler.provideToSp(1_508.113441559160422894 ether, false);

        // totalEvroDeposits = 110_115.161460252303371681 ether

        vm.prank(barb);
        handler.provideToSp(0.000000000000017716 ether, false);

        // totalEvroDeposits = 110_115.161460252303389397 ether
        ...
        // totalEvroDeposits = 17_525.230265653071151161 ether
        ...
        // totalEvroDeposits = 15_384.386607767348466801 ether
    ...
    function testNotEnoughYieldToClaim4() external {
        ...
        // totalEvroDeposits = 98_018.79452055664212293 ether

        vm.prank(dana);
        handler.liquidateMe();

        // totalEvroDeposits = 9.397260282669519962 ether
        ...
        // totalEvroDeposits = 9.397260282669541918 ether
        ...
        // totalEvroDeposits = 50_247.981212687337758717 ether
        ...
        // totalEvroDeposits = 89_544.122851108357497128 ether
        ...
        // totalEvroDeposits = 89_544.122851108357497129 ether

        vm.prank(barb);
        handler.liquidateMe();

        // totalEvroDeposits = 87_543.931070286439688907 ether
    ...
    function testCollGainsUnderflow3CollSkin() external {
        ...
        // totalEvroDeposits = 3.702318759892672893 ether

        // pulling `deposited` from fixture
        vm.prank(carl);
        handler.provideToSp(7.404637519785345786 ether, false);

        // totalEvroDeposits = 11.106956279678018679 ether

        vm.prank(carl);
        handler.provideToSp(6_872.312325153568231613 ether, false);

        // totalEvroDeposits = 6_883.419281433246250292 ether

        // pulling `deposited` from fixture
        vm.prank(carl);
        handler.provideToSp(6_884.455930686016187896 ether, false);

        // totalEvroDeposits = 13_767.875212119262438188 ether
        ...
        // totalEvroDeposits = 70_270.357539205627400143 ether
        ...
        // totalEvroDeposits = 136_843.345806819783962098 ether

        vm.prank(adam);
        handler.liquidateMe();

        // totalEvroDeposits = 72_060.66754338042049397 ether
        ...
        // totalEvroDeposits = 136_843.345806819784026881 ether

        vm.prank(eric);
        handler.liquidateMe();

        // totalEvroDeposits = 38_824.550385164689247862 ether
        // P = 0.149402322152386736 ether

        vm.prank(dana);
        handler.liquidateMe();

        // totalEvroDeposits = 210.952427524467700142 ether
        ...
        // totalEvroDeposits = 370_619.739196103579284353 ether

        vm.prank(barb);
        handler.liquidateMe();

        // totalEvroDeposits = 272_610.340127738994143277 ether
        ...
        // totalEvroDeposits = 273_119.029872486374576994 ether

        vm.prank(eric);
        handler.provideToSp(0.000000000000011519 ether, false);

        // totalEvroDeposits = 273_119.029872486374588513 ether

        vm.prank(dana);
        handler.provideToSp(14_325.409601730288741627 ether, false);

        // totalEvroDeposits = 287_444.43947421666333014 ether

        vm.prank(hope);
        handler.liquidateMe();

        // totalEvroDeposits = 189_425.95145220850566442 ether
        // P = 0.000393490982450372 ether

        vm.prank(carl);
        handler.provideToSp(656.318601037450927984 ether, false);

        // totalEvroDeposits = 190_082.270053245956592404 ether
        ...
        // totalEvroDeposits = 228_695.868010886178178738 ether

        vm.prank(dana);
        handler.provideToSp(53_385.94712175149302094 ether, false);

        // totalEvroDeposits = 282_081.815132637671199678 ether

        vm.prank(carl);
        handler.provideToSp(6_856.901188404296809837 ether, false);

        // totalEvroDeposits = 288_938.716321041968009515 ether
        ...
        // totalEvroDeposits = 294_820.222908736783086572 ether
        ...
        // totalEvroDeposits = 392_839.01833039187796361 ether

        vm.prank(eric);
        handler.provideToSp(1_186.940091321995741882 ether, false);

        // totalEvroDeposits = 394_025.958421713873705492 ether
    ...
    function testSPYieldBigDispropRedeem() external {
        ...
        // totalEvroDeposits = 65_346_446_343_250_241_630.04102517305741141 ether

        vm.prank(eric);
        handler.provideToSp(0.00000000000000052 ether, false);

        // totalEvroDeposits = 65_346_446_343_250_241_630.04102517305741193 ether

        vm.prank(barb);
        handler.liquidateMe();

        // totalEvroDeposits = 65.346446343250242085 ether
        ...
        // totalEvroDeposits = 67_005_212_327_471_008_664.322537884636873174 ether
