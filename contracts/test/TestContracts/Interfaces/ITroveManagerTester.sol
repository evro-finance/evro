...
interface ITroveManagerTester is ITroveManager {
    ...
    function getCollGasCompensation(uint256 _coll, uint256 _debt, uint256 _evroInSPForOffsets)
        external
        pure
        returns (uint256);
    ...
    function getPendingEvroDebtReward(uint256 _troveId) external view returns (uint256);
